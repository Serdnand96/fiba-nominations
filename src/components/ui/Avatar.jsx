...
// o si la imagen no carga, cae a las iniciales de `name`.
export function Avatar({ name, src, size='md', tone='navy', className='' }) {
  ...
  if (src && !broken) {
    return (
      <span className={`${base} bg-ink-100 dark:bg-navy-800`}>
        <img src={src} alt={name || ''} className="w-full h-full object-cover" loading="lazy" onError={() => setBroken(true)} />
      </span>
    );
  }
  return <span className={`${base} ${tones[tone]}`}>{initials || '—'}</span>;
}
