...
const DEFAULT_EVENT_ID = 'a0000000-0000-0000-0000-000000000001'
...
export default function Transport() {
  ...
  const [passengerForm, setPassengerForm] = useState({ name: '', hotel: 'Hotel Albrook', category: '' })
  ...
  const tripsByVehicle = useMemo(() => {
    ...
    for (const trip of tripData.trips) {
      if (map[trip.vehicle_id]) {
        map[trip.vehicle_id].trips.push(trip)
      }
    }
    for (const vd of tripData.vehicle_drivers) {
      if (map[vd.vehicle_id]) {
        map[vd.vehicle_id].driver = vd.driver
      }
    }
    // Sort trips within each vehicle by departure_time
    Object.values(map).forEach(g => g.trips.sort((a, b) => a.departure_time?.localeCompare(b.departure_time)))
    return map
  }, [tripData])

  // Conflict trip IDs for highlighting
  ...
  const hotels = useMemo(() => {
    ...
    return hotelVenues.length > 0 ? hotelVenues.map(v => v.name) : ['Hotel Albrook', 'Hotel Milan']
  }, [venues])
  ...
  function openCreateTrip(vehicleId) {
    ...
    const nextNum = existingTrips.length + 1
    setEditingTrip(null)
    setTripForm({
      vehicle_id: vehicleId, trip_number: nextNum, departure_time: '',
  ...
  async function handleDeleteTrip(trip) {
    const label = lang === 'es' ? '¿Eliminar este viaje?' : 'Delete this trip?'
  ...
  async function handlePassengerSubmit(e) {
    ...
    setPassengerForm({ name: '', hotel: 'Hotel Albrook', category: '' })
  ...
  function exportPDF() {
    ...
    <title>Transporte - ${dateLabel}</title>
    <style>
    ...
    <div class="header">
      <h1>TRANSPORTE - JUEGOS ODESUR PANAM&Aacute; 2026</h1>
      <h2>${dateLabel}</h2>
    </div>
    <div class="container">
    <div class="main">`
      ...
      {tab === 'schedule' && (
        <div>
          ...
          <div className="flex items-center gap-3 mb-6">
            <button onClick={() => setDate(d => { const dt = new Date(d + 'T12:00:00'); dt.setDate(dt.getDate() - 1); return dt.toISOString().split('T')[0] })}
            ...
            <span className="text-sm text-gray-500">
              {new Date(date + 'T12:00:00').toLocaleDateString(lang === 'es' ? 'es' : 'en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
            </span>
            {conflicts.length > 0 && (
          ...
          <div className="space-y-4">
                        ...
                        <tr key={trip.id} className={`hover:bg-gray-50 ${conflictTripIds.has(trip.id) ? 'bg-red-50' : ''}`}>
                          <td className="px-4 py-2 font-medium">
                            {conflictTripIds.has(trip.id) && (
                              <span className="inline-block w-2 h-2 rounded-full bg-red-500 mr-1.5" title={lang === 'es' ? 'Conflicto de horario' : 'Schedule conflict'} />
                            )}
                          ...
                          <td className="px-4 py-2">
                            {trip.equipment && (
                              <span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded text-xs">{trip.equipment}</span>
                            )}
                          </td>
            ...
            {Object.keys(tripsByVehicle).length === 0 && (
              <div className="text-center py-12 text-gray-400">
                <p className="text-sm">{lang === 'es' ? 'No hay vehículos configurados.' : 'No vehicles configured.'}</p>
            ...
            <div className="bg-white rounded-lg border overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  ...
                  {vehicles.length === 0 && (
                    <tr><td colSpan={3} className="px-4 py-6 text-center text-gray-400 text-xs">{lang === 'es' ? 'Sin vehículos' : 'No vehicles'}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className="bg-white rounded-lg border overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  ...
                  {drivers.length === 0 && (
                    <tr><td colSpan={3} className="px-4 py-6 text-center text-gray-400 text-xs">{lang === 'es' ? 'Sin choferes' : 'No drivers'}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                  ...
                  <div key={cat}>
                    ...
                    {people.map(p => (
                      <div key={p.id} className="flex items-center justify-between px-4 py-1.5 border-b border-gray-50 hover:bg-gray-50">
                        ...
                        <button onClick={() => handleDeletePassenger(p)} className="text-red-400 hover:text-red-600 text-xs">&times;</button>
                      </div>
                    ))}
                  </div>
                ))}
