...
import {
  ...
  getTrainingSlots, getTrainingSlotsByDate, getTrainingSlotsByTeam,
  getTrainingSlotsByPersonnel, createTrainingSlot, updateTrainingSlot,
...
export default function Training() {
  ...
  async function loadSlots() {
    try {
      ...
      // Auto-select first date if none set
  ...
  // Derived data
  const dates = useMemo(() => {
    return [...new Set(allSlots.map(s => s.date))].sort()
  }, [allSlots])

  const teams = useMemo(() => {
    return [...new Set(allSlots.map(s => s.team_label))].sort()
  }, [allSlots])
  ...
  const teamSlots = useMemo(() => {
    ...
    return allSlots.filter(s => s.team_label === selectedTeam).sort((a, b) => a.date.localeCompare(b.date) || a.start_time.localeCompare(b.start_time))
  }, [allSlots, selectedTeam])

  const tdSlots = useMemo(() => {
    if (!selectedTd) return []
    return allSlots.filter(s =>
      s.assignments?.some(a => a.personnel_id === selectedTd)
  ...
  const availableTds = useMemo(() => {
    return tds.filter(td => {
      ...
      if (!entry) return true // no data = show
  ...
  function openCreateSlot() {
    setEditingSlot(null)
    setSlotForm({
      date: selectedDate || '', start_time: '', end_time: '', venue: 'Estadio',
      team_label: '', sport: 'Basketball', notes: '',
    })
  ...
  async function handleSlotSubmit(e) {
    ...
    try {
      if (editingSlot) {
        await updateTrainingSlot(editingSlot.id, slotForm)
      } else {
        await createTrainingSlot({ ...slotForm, competition_id: competitionId })
      }
      setShowSlotModal(false)
      await loadSlots()
      showToast(editingSlot ? t('training.slotUpdated') : t('training.slotCreated'), 'green')
    } catch (err) {
  ...
  async function handleDeleteSlot(slot) {
    const label = lang === 'es' ? '¿Eliminar este slot de entrenamiento?' : 'Delete this training slot?'
  ...
  async function handleAssign() {
    ...
    try {
      const result = await createTrainingAssignment({
        training_slot_id: assignSlot.id,
        personnel_id: assignTdId,
      })
      if (result.conflict_warning) {
        showToast(`${t('training.assignedWithConflict')}: ${result.conflict_warning}`, 'yellow')
      } else {
        showToast(t('training.tdAssigned'), 'green')
      }
      setAssignTdId('')
      await loadSlots()
      // Refresh the assign modal slot data
      const updated = allSlots.find(s => s.id === assignSlot.id)
      if (updated) setAssignSlot(null) // close and reopen if needed
      setAssignSlot(null)
    } catch (err) {
  ...
  async function handleRemoveAssignment(assignmentId) {
    ...
    await loadSlots()
    showToast(t('training.tdRemoved'), 'green')
    setAssignSlot(null)
  }
  ...
  function openImport() {
    setShowImport(true)
    setImportCompId(competitionId)
    setImportSport('Basketball')
    setImportFile(null)
  ...
  async function handlePreview() {
    ...
    setImporting(true)
    try {
      const result = await previewTrainingExcel(importFile, importCompId, importSport)
  ...
  async function handleImport() {
    ...
    setImporting(true)
    try {
      const result = await importTrainingExcel(importFile, importCompId, importSport)
  ...
  const tabs = [
    { key: 'byDay', label: t('training.byDay') },
    { key: 'byTeam', label: t('training.byTeam') },
    { key: 'byTd', label: t('training.byTd') },
  ]
        ...
        {tabs.map(tb => (
          <button key={tb.key} onClick={() => setTab(tb.key)}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
              tab === tb.key ? 'bg-white shadow text-blue-700' : 'text-gray-600 hover:text-gray-900'
            }`}>
            {tb.label}
          </button>
        ))}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {daySlots.estadio.length === 0 ? (
              <p className="text-xs text-gray-400">{t('training.noSlots')}</p>
            ) : (
              <div className="space-y-2">
                {daySlots.estadio.map(slot => <SlotCard key={slot.id} slot={slot} />)}
              </div>
            )}
          </div>
          <div>
            <h3 className="text-sm font-bold text-gray-600 mb-2 uppercase">{t('training.trainingCourt')}</h3>
            {daySlots.cancha.length === 0 ? (
              <p className="text-xs text-gray-400">{t('training.noSlots')}</p>
            ) : (
              <div className="space-y-2">
                {daySlots.cancha.map(slot => <SlotCard key={slot.id} slot={slot} />)}
              </div>
            )}
          </div>
        </div>
  ...
  function SlotCard({ slot }) {
    ...
    return (
      <div className="bg-white border rounded-lg p-3 hover:shadow-sm transition-shadow cursor-pointer"
        onClick={() => canEdit ? openAssignModal(slot) : null}>
        <div className="flex items-center justify-between mb-1">
          ...
          {canEdit && (
            <div className="flex gap-1">
              <button onClick={e => { e.stopPropagation(); openEditSlot(slot) }}
                    ...
                    <tr key={slot.id} className="hover:bg-gray-50">
                      ...
                      {canEdit && (
                        <td className="px-4 py-2 text-right">
                          <button onClick={() => openAssignModal(slot)} className="text-blue-600 hover:underline text-xs mr-2">
                            {t('training.assign')}
                          </button>
                          <button onClick={() => openEditSlot(slot)} className="text-gray-500 hover:underline text-xs mr-2">
                            {t('common.edit')}
                          </button>
                          <button onClick={() => handleDeleteSlot(slot)} className="text-red-500 hover:underline text-xs">
                            {t('common.delete')}
                          </button>
                        </td>
                      )}
        ...
        <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
          ...
          <form onSubmit={handleSlotSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => setShowSlotModal(false)} className="px-4 py-2 text-sm text-gray-600">
                {t('common.cancel')}
              </button>
              <button type="submit"
  ...
  function AssignmentModal() {
    const assigned = assignSlot.assignments || []
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
          ...
          {canEdit && (
            <div>
              ...
              <div className="flex gap-2">
                <select value={assignTdId} onChange={e => setAssignTdId(e.target.value)}
                  className="flex-1 px-3 py-2 border rounded-lg text-sm">
                  <option value="">{t('training.selectTd')}</option>
                  {availableTds
                    .filter(td => !assigned.some(a => a.personnel_id === td.id))
                    .map(td => (
                      <option key={td.id} value={td.id}>{td.name}{td.country ? ` (${td.country})` : ''}</option>
                    ))}
                </select>
        ...
        <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
          ...
          {importResult ? (
                ...
                {importResult.errors?.length > 0 && (
                  <div className="mt-2">
                    ...
                    {importResult.errors.map((e, i) => (
                      <p key={i} className="text-xs text-red-500">{e.team_label}: {e.error}</p>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('training.sport')}</label>
                <input type="text" value={importSport} onChange={e => setImportSport(e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg text-sm" />
              </div>
              <div>
              ...
              {importPreview && (
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs font-medium text-gray-600 mb-2">
                    {t('training.preview')}: {importPreview.total} slots
                  </p>
                  <div className="max-h-40 overflow-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="text-gray-500">
                          <th className="text-left px-2 py-1">{t('training.date')}</th>
                          <th className="text-left px-2 py-1">{t('training.start')}</th>
                          <th className="text-left px-2 py-1">{t('training.venue')}</th>
                          <th className="text-left px-2 py-1">{t('training.team')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {importPreview.preview.map((s, i) => (
                          <tr key={i} className="border-t">
                            ...
                            <td className="px-2 py-1">{s.team_label}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowImport(false)} className="px-4 py-2 text-sm text-gray-600">
                  {t('common.cancel')}
                </button>
                {!importPreview ? (
